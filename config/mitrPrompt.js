...
export function buildMitrSystemPrompt({
  ...
  if (Array.isArray(recalledMemories) && recalledMemories.length > 0) {
    memoryContext = recalledMemories.map((memory, index) => `${index + 1}. ${memory}`).join("\n");
  }
  ...
  let cognitiveContextText = "No current cognitive-game performance data is available.";

  if (cognitiveContext) {
    cognitiveContextText = `
- Current cognitive level: ${cognitiveContext.level ?? "unknown"}
- Recent game score: ${cognitiveContext.score ?? "unknown"}
- Accuracy: ${cognitiveContext.accuracy ?? "unknown"}
- Average response time: ${cognitiveContext.responseTime ?? "unknown"}
- Recent consistency: ${cognitiveContext.consistency ?? "unknown"}
- Recommended difficulty: ${cognitiveContext.recommendedDifficulty ?? "unknown"}
`;
  }
  ...
  if (Array.isArray(activeReminders) && activeReminders.length > 0) {
    reminderContext = activeReminders.map((reminder, index) => `${index + 1}. ${reminder}`).join("\n");
  }
  ...
  let caregiverContextText = "No caregiver-specific information is available.";

  if (caregiverContext) {
    caregiverContextText = `
- Caregiver relationship: ${caregiverContext.relationship ?? "unknown"}
- Caregiver name: ${caregiverContext.name ?? "unknown"}
`;
  }
  ...
  let sessionContextText = "No additional session context is available.";

  if (sessionContext) {
    sessionContextText = JSON.stringify(sessionContext, null, 2);
  }
